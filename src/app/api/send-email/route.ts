  ...
  try {
    ...
    if (type === 'welcome') {
      ...
      if (!welcomePayload.success) {
        ...
        return NextResponse.json({ success: false, message: 'Invalid welcome email payload.' }, { status: 400 });
      }
      ...
      const confirmationPayload = ConfirmationEmailInputSchema.safeParse(payload);
       if (!confirmationPayload.success) {
        ...
        return NextResponse.json({ success: false, message: 'Invalid confirmation email payload.' }, { status: 400 });
      }
