...
/**
 * @fileOverview An email sending service using Resend for activity registration.
 ...
 */
export async function sendConfirmationEmail(input: ConfirmationEmailInput): Promise<EmailOutput> {
  const resendApiKey = process.env.RESEND_API_KEY;

  console.log('--- Activity Confirmation Email Flow Started ---');
  console.log('[EMAIL FLOW INFO] Input received:', JSON.stringify(input, null, 2));

  if (!resendApiKey || resendApiKey.trim() === '') {
      console.error('[EMAIL FLOW ERROR] RESEND_API_KEY is not configured or is empty.');
      console.log('--- Activity Confirmation Email Flow Ended with Error ---');
      return {
          success: false,
  ...
  try {
      ...
      if (error) {
          ...
          console.log('--- Activity Confirmation Email Flow Ended with API Error ---');
          return { success: false, message: `Failed to send email: ${error.message}` };
      }

      console.log('[EMAIL FLOW SUCCESS] Email sent successfully. Response:', JSON.stringify(data, null, 2));
      console.log('--- Activity Confirmation Email Flow Ended Successfully ---');
      ...
      console.log('--- Activity Confirmation Email Flow Ended with Exception ---');
